...
\subsection*{2 --- Get the starter sketch}

On Home, tap the docs icon on \ui{Control panel} (or open
\ui{Codes and Documents} from that module).
Share or save the DevKit Classic ZIP, copy it to the computer, and open the
\texttt{.ino} folder in Arduino IDE.

\shot{S02}{s02_codes.png}{Portrait, light theme}
{Codes and Documents for Control panel. The general user-guide PDF, the
fast guide, and the ESP32 Classic ZIP row must all be visible.
English UI.}
...
\subsection*{3 --- Match settings, then connect}
...
Leave the screen, then tap the Bluetooth button on the module toolbar.

\shot{S03}{s03_settings_default.png}{Portrait, light theme, scrolled to User type}
{Control panel settings. User type Default selected. Connection section
showing Bluetooth + Classic + Simple (text lines). No Advanced options
expanded.}
...
\rowcolor{TblTelem}
Docs icon on a card &
  ZIP sketches and PDFs for that module. \\
Gold \ui{PRO} &
...
\begin{itemize}[leftmargin=1.4cm]
  \item \ui{Codes and Documents} --- this PDF, a short fast guide, and the
        ZIP that matches the board + mode you selected in settings.
...
\ui{RC Vehicle} entry, Advanced links, and extra center views
(Camera, Radar, session CSV) use coins or a Pro subscription.

The gold sparkle on a locked card is an explorer gift: a short ad unlocks
that Pro module for a few hours.

\shot{S14}{s14_coin_unlock.png}{Portrait, unlock dialog}
{Coin unlock dialog for RC Vehicle or Advanced settings. Show coin price
options and the module name. English UI.}
...
Replace each yellow box with the file named in the header.
Prefer English UI for the English PDF (Spanish UI for the Spanish PDF),
light theme, cropped to the app (except S04 and S09, which are system sheets).
...
\thc{File} & \thc{Shot} \\
S01 & s01\_home.png & Home, not connected, both module cards. \\
\rowcolor{TblTelem}
S02 & s02\_codes.png & Codes \& Documents, Control panel. \\
S02b & s01\_serial.png & Arduino Serial Monitor, Classic Simple banner. \\
\rowcolor{TblTelem}
S03 & s03\_settings\_default.png & Settings: Default + Classic Simple. \\
\rowcolor{TblTelem}
S04 & s04\_permission.png & Android Bluetooth / Nearby dialog. \\
S05 & s05\_bluetooth\_scan.png & Scan list with ESP32-TC-RC-BT-Simple. \\
\rowcolor{TblTelem}
S06 & s06\_home\_connected.png & Home, Connected. \\
S07 & s07\_control\_panel.png & Control panel landscape HUD. \\
\rowcolor{TblTelem}
S08 & s08\_home\_help.png & Home Quick help dialog. \\
S09 & s09\_wifi\_join.png & System Wi-Fi joining CAM SoftAP. \\
\rowcolor{TblTelem}
S10 & s10\_panel\_settings.png & Joystick Spring / Hold. \\
S11 & s11\_center\_picker.png & Center display picker. \\
\rowcolor{TblTelem}
S12 & s12\_rc\_vehicle.jpg & RC Vehicle HUD + camera. \\
S13 & s13\_vehicle\_settings.png & RC Vehicle Default CAM settings. \\
\rowcolor{TblTelem}
S14 & s14\_coin\_unlock.png & Coin unlock dialog. \\
\end{tabular}
\end{center}
