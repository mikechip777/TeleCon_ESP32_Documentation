/*
 * TeleCon_Classic_Binary
 * RC Vehicle Pro — Classic SPP + Binary RC (AA 55 / BB 66 / CC 11/22/33).
 */
...
void setup() {
  ...
  if (!teleconBluetoothBegin()) {
    DBG_PRINTLN("Bluetooth init failed!");
    while (true) { delay(1000); }
  }

  DBG_PRINTLN("TeleCon RC Vehicle Pro Classic Binary ready.");
  DBG_PRINTF("Bluetooth: %s\n", TELECON_BT_NAME);
  DBG_PRINTLN("Android: RC Vehicle Pro → Board DevKit → Connection = Classic + Binary");
