/*
 * RcVehicleHandlers.cpp — CONNECT / AA55 / BB66 / CC telemetry over Classic SPP.
 *
...
bool sendPanelPacket(uint16_t left, uint16_t right, uint8_t flags) {
  ...
  return teleconSendBinary(pkt, TELECON_PANEL_SIZE);
}
...
bool sendIndicatorPacket(uint8_t analog, uint8_t batt, uint8_t led) {
  ...
  return teleconSendBinary(pkt, TELECON_INDICATOR_SIZE);
}
...
bool sendPlotPacket(uint8_t v0, uint8_t v1, uint8_t v2, uint8_t v3) {
  ...
  buf[idx++] = cs;
  return teleconSendBinary(buf, (size_t)idx);
}
...
bool canSendTelemetry() {
  return handshakeConfirmed && SerialBT.hasClient();
}
...
void handleRcSetText(const String& line) {
  ...
  teleconSendLine("RC:ACK,steer_center,1");
#if TELECON_DEBUG
