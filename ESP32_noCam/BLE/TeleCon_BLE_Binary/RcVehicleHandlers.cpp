/*
 * RcVehicleHandlers.cpp — CONNECT / AA55 / BB66 / CC telemetry over BLE NUS.
 *
...
void sendRcAck() {
  teleconBleSendLine("RC:ACK,app,RC");
  ...
  lastBatt = 0xFF;
  lastLed = 0xFF;
  teleconBleSetNotificationsEnabled(true);
...
bool sendPanelPacket(uint16_t left, uint16_t right, uint8_t flags) {
  ...
  return teleconBleSendBinary(pkt, TELECON_PANEL_SIZE);
}
...
bool sendIndicatorPacket(uint8_t analog, uint8_t batt, uint8_t led) {
  ...
  return teleconBleSendBinary(pkt, TELECON_INDICATOR_SIZE);
}
...
bool sendPlotPacket(uint8_t v0, uint8_t v1, uint8_t v2, uint8_t v3) {
  ...
  buf[idx++] = cs;
  return teleconBleSendBinary(buf, (size_t)idx);
}
...
bool canSendTelemetry() {
  return handshakeConfirmed &&
         teleconBleIsConnected() &&
         teleconBleNotificationsEnabled();
}
...
void handleRcSetText(const String& line) {
  ...
  teleconBleSendLine("RC:ACK,steer_center,1");
#if TELECON_DEBUG
...
bool rcVehicleHandleSerialInjectLine(const String& line) {
  if (!teleconBleIsConnected()) {
    DBG_PRINTLN("[SERIAL] no BLE client — line not sent");
    return false;
  }
  ...
  if (!teleconBleNotificationsEnabled()) {
    teleconBleSetNotificationsEnabled(true);
  }
